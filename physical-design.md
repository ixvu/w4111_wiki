...
-Commonly, it includes index entries and data entries. In the primary index structure, the data entries is the data record and we can directly access the turple using it; however, in secondary index structure, the data entries has the following format <search key value, rid> and we have another layer called data records. The actual record is stored in that layer.
###B+ tree Index
-Each node of it represent for a page. It is self-balanced, therefore, it can keep a good search performance. Besides, leaf nodes are connected as the following picture showed. This data structure help to actualize the disk optimition.
